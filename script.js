...
(function() {
  ...
  let currentIdx = 0, isPlaying = false, userInteracted = false;

  function loadTrack(idx) {
    currentIdx = ((idx % PLAYLIST.length) + PLAYLIST.length) % PLAYLIST.length;
    audio.src = BASE_PATH + PLAYLIST[currentIdx].file;
    mpTitle.textContent = PLAYLIST[currentIdx].title;
  ...
  function play() {
    audio.play().then(() => {
      isPlaying = true;
      btnPlay.innerHTML = '&#9646;&#9646;';
      if (mpIcon) mpIcon.classList.add('spinning');
    }).catch(() => {});
  }
  ...
  function pause() {
    ...
    isPlaying = false;
    btnPlay.innerHTML = '&#9654;';
    if (mpIcon) mpIcon.classList.remove('spinning');
  }

  btnPlay.addEventListener('click', () => { userInteracted = true; isPlaying ? pause() : play(); });
  btnNext.addEventListener('click', () => { loadTrack(currentIdx + 1); if (isPlaying) play(); });
  btnPrev.addEventListener('click', () => {
    if (audio.currentTime > 3) audio.currentTime = 0;
    else { loadTrack(currentIdx - 1); if (isPlaying) play(); }
  });
  audio.addEventListener('ended', () => { loadTrack(currentIdx + 1); play(); });
  ...
  loadTrack(0);
  document.addEventListener('click', () => { if (!userInteracted) { userInteracted = true; play(); } }, { once: true });
})();
